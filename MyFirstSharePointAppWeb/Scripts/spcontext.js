(function (window, undefined) {
    ...
    $(document).ready(function () {
        ...
        var appweburl = getQueryStringParameter(SPAppUrlKey);

        if (spHostUrl && currentAuthority) {
            appendSPHostUrlToLinks(spHostUrl, appweburl, currentAuthority);
        }
    });

    // Appends SPHostUrl as query string to all the links which point to current domain.
    function appendSPHostUrlToLinks(spHostUrl, appweburl, currentAuthority) {
        $("a")
            .filter(function () {
            ...
            .each(function () {
                if (!getSPHostUrlFromQueryString(this.search)) {
                    if (this.search.length > 0) {
                        this.search += "&" + SPHostUrlKey + "=" + spHostUrl + "&" + SPAppUrlKey + "=" + appweburl;
                    }
                    else {
                        this.search = "?" + SPHostUrlKey + "=" + spHostUrl + "&" + SPAppUrlKey + "=" + appweburl
                    }
